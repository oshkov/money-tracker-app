...
from src.models import Account, Category, Operation
...
# Создание счета
async def create_account(session, user, account: AccountCreate):
...
# Создание категории
async def create_category(session, user, category: CategoryCreate):
...
# Создание операции
async def create_operation(session, user, operation: OperationCreate):
